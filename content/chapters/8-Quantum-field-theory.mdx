...
Take any free classical field, Fourier expand it into modes labeled by momentum $\mathbf k$. Each mode is a harmonic oscillator. Quantization then is copy–paste from Chapter 7, but for every $\mathbf k$. That is the whole vibe: **quantize all the normal modes**.

---

### 8.1.3 Classical warm-up: the Klein–Gordon field

For a real scalar field $\phi(x)$ with $x=(ct,\mathbf x)$, the Lorentz-invariant action is
...
$$
\hat\phi(x) = \frac{1}{\sqrt{V}}\sum_{\mathbf k} \frac{1}{\sqrt{2\omega_{\mathbf k}}}\left(\hat a_{\mathbf k}\,e^{-i\omega_{\mathbf k} t + i\mathbf k\cdot\mathbf x} + \hat a_{\mathbf k}^{\dagger}\,e^{i\omega_{\mathbf k} t - i\mathbf k\cdot\mathbf x}\right)
$$

with

$$
\omega_{\mathbf k} = \sqrt{c^{2}\mathbf k^{2} + \frac{m^{2}c^{4}}{\hbar^{2}}}
$$

Choose the normalization so that

$$
[\hat a_{\mathbf k},\hat a_{\mathbf k'}^{\dagger}] = \delta_{\mathbf k\mathbf k'},\qquad
[\hat a_{\mathbf k},\hat a_{\mathbf k'}] = 0,\qquad
...
Define the vacuum by $\hat a_{\mathbf k}\ket{0}=0$. **One-particle** states are

$$
\ket{\mathbf k} \equiv \hat a_{\mathbf k}^{\dagger}\ket{0}
$$

and multi-particle states follow by applying more creation operators. The **number operator** for mode $\mathbf k$ is $\hat N_{\mathbf k}=\hat a_{\mathbf k}^{\dagger}\hat a_{\mathbf k}$. The Hamiltonian of the free field is
...
$$
:\hat a_{\mathbf k}^{\dagger}\hat a_{\mathbf k}: = \hat a_{\mathbf k}^{\dagger}\hat a_{\mathbf k}
$$

and

$$
:\hat H: = \sum_{\mathbf k}\hbar\omega_{\mathbf k}\,\hat a_{\mathbf k}^{\dagger}\hat a_{\mathbf k}
$$
...
Show $\langle \mathbf k'|\mathbf k\rangle = \delta_{\mathbf k'\mathbf k}$ using the $[\hat a,\hat a^{\dagger}]$ algebra.
...
Act with $\hat H$ on $\ket{\mathbf k}$ and confirm $E=\hbar\omega_{\mathbf k}$.
...
Klein–Gordon waves handle relativity but describe spin-0 and permit negative densities. We want a **first-order** equation in time with a positive-definite probability density, consistent with $E^{2}=\mathbf p^{2}+m^{2}$. Dirac’s idea: linearize the relativistic dispersion with matrices obeying a new algebra.
...
$$
(\slashed{p} - m)\,u_{s}(p) = 0,\qquad (\slashed{p} + m)\,v_{s}(p) = 0
$$

where $\slashed{p}\equiv \gamma^{\mu}p_{\mu}$ and $s$ labels spin. Normalizations convenient for QFT:
...
$$
\sum_{s} u_{s}(p)\,\bar u_{s}(p) = \slashed{p} + m
$$

$$
\sum_{s} v_{s}(p)\,\bar v_{s}(p) = \slashed{p} - m
$$
