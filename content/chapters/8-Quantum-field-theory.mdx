...
$$
\boldsymbol{\mu} = g\,\frac{e}{2m}\,\boldsymbol S,\qquad g_{\text{Dirac}}=2
$$
...
$$
\hat\psi(x) = \sum_{s}\int\!\frac{d^{3}\boldsymbol p}{(2\pi)^{3}}\;\frac{1}{\sqrt{2E_{\boldsymbol p}}}\left[
\hat b_{s}(\boldsymbol p)\,u_{s}(p)\,e^{-ip\cdot x} + \hat d_{s}^{\dagger}(\boldsymbol p)\,v_{s}(p)\,e^{+ip\cdot x}\right]
$$

with $E_{\boldsymbol p}=\sqrt{\boldsymbol p^{2}+m^{2}}$ and

$$
\{\hat b_{s}(\boldsymbol p),\hat b_{s'}^{\dagger}(\boldsymbol p')\}=(2\pi)^{3}\delta_{ss'}\delta^{(3)}(\boldsymbol p-\boldsymbol p')
$$

$$
\{\hat d_{s}(\boldsymbol p),\hat d_{s'}^{\dagger}(\boldsymbol p')\}=(2\pi)^{3}\delta_{ss'}\delta^{(3)}(\boldsymbol p-\boldsymbol p')
$$
...
$$
:\hat H: = \sum_{s}\int\!\frac{d^{3}\boldsymbol p}{(2\pi)^{3}}\;E_{\boldsymbol p}\left[\hat b_{s}^{\dagger}(\boldsymbol p)\hat b_{s}(\boldsymbol p)+\hat d_{s}^{\dagger}(\boldsymbol p)\hat d_{s}(\boldsymbol p)\right]
$$
...
$$
S_{F}(p) = \frac{i}{/\!\!p - m + i\epsilon} = i\,\frac{/\!\!p+m}{p^{2}-m^{2}+i\epsilon}
$$
...
$$
H_{\text{Pauli}} = \frac{1}{2m}\,(\boldsymbol p - e\boldsymbol A)^{2} + e\phi - \frac{e}{2m}\,\boldsymbol{\sigma}\cdot \boldsymbol B
$$

with $\boldsymbol B=\nabla\times\boldsymbol A$. The $\boldsymbol{\sigma}\cdot\boldsymbol B$ term gives $g=2$ at tree level. Higher orders add spin–orbit and Darwin terms (matching hydrogen fine structure).
...
Check that $(/\!\!p-m)\,[(/\!\!p+m)/(p^{2}-m^{2})] = \mathbb I$.

**(c) Spin sums.**  
Prove $\sum_{s}u_{s}(p)\bar u_{s}(p)=/\!\!p+m$ by completeness of plane-wave solutions.

**(d) Pauli reduction.**  
Starting from $(\boldsymbol{\alpha}\cdot\boldsymbol{\pi}+ \beta m + e\phi)\psi=i\partial_{t}\psi$, with $\boldsymbol{\pi}=\boldsymbol p-e\boldsymbol A$, eliminate the small component to order $1/m$ and recover $-\tfrac{e}{2m}\boldsymbol{\sigma}\cdot\boldsymbol B$.

**(e) Helicity conservation for $m=0$.**  
Show that $[H,\boldsymbol{\Sigma}\cdot \hat{\boldsymbol p}]=0$ for the free massless Dirac Hamiltonian, so helicity is conserved.
...
$$
q_{\mu}\,\mathcal M^{\mu} = \bar u(p')\,(/\!\!{p'}-/\!\!p)\,u(p)
$$

Insert $(/\!\!p-m)u(p)=0$ and $\bar u(p/\!\!{p'}-m)=0$ to get
...
- **On-shell limit**: $p^{2}\to m^{2}$ (or $/\!\!p\to m$ for fermions)  
...
$$
\tilde S(p) \equiv \int d^{4}x\,e^{ip\cdot x}\,\langle 0|T\{\psi(x)\bar\psi(0)\}|0\rangle \;\sim\; \frac{i\,Z_{\psi}(/\!\!p+m)}{p^{2}-m^{2}+i\epsilon}
$$
...
$$
\mathcal M \;\propto\; Z_{\psi}^{-1/2}\, \bar u(p') \left[ -i\!\int d^{4}y\,e^{-ip'\cdot y}\,\left(i\overleftarrow{/\!\!partial}_{y}+m\right)\right]
\,\langle 0|T\{\psi(y)\cdots\bar\psi(x)\}|0\rangle\,
\left[i\!\int d^{4}x\,e^{ip\cdot x}\,\left(i/\!\!partial_{x}-m\right)\right] Z_{\psi}^{-1/2}\, u(p)
$$
...
Show that inserting $(i\overleftarrow{/\!\!partial}+m)$ on an outgoing $\psi$ line and contracting with $\bar u(p')$ reproduces the usual external spinor factor when $p'^{2}=m^{2}$.
...
- **Fermion propagator**: $i(/\!\! p + m)/(p^{2}-m^{2}+i\epsilon)$  
...
**Electron self-energy** $\Sigma(/\!\! p)$ renormalizes mass and wavefunction. In $\overline{\mathrm{MS}}$ at one loop,

$$
\Sigma(\/\!\! p) = \frac{\alpha}{4\pi}\left[\left(\frac{1}{\epsilon}-\gamma_{E}+\ln 4\pi\right)(-/\!\! p + 4m) + \cdots\right]
$$
...
$$
\sum_{s}u^{s}(p)\bar u^{s}(p)=/\!\! p+m,\qquad \sum_{s}v^{s}(p)\bar v^{s}(p)=/\!\! p-m
$$
...
after regulating $\mathrm{Tr}\,\gamma^{5}\exp(-/\!\! D^{2}/\Lambda^{2})$. That produces the same ABJ result and generalizes to non-Abelian cases.
...
**(b) Fujikawa trace**  
Compute $\mathrm{Tr}\,\gamma^{5}\exp(-\not D^{2}/\Lambda^{2})$ to leading order in $1/\Lambda$ and extract $F\tilde F$
...
<RelatedBooks
  items={[
    { title: "Quantum Field Theory: A Very Short Introduction", author: "Tom Lancaster", note: "Introductory Level", href: "https://amzn.to/45YIrYk" },
    { title: "An Introduction to Quantum Field Theory", author: "Peskin & Schroeder", note: "Classic Work", href: "https://amzn.to/45SkqBX" },
  ]}
/>
