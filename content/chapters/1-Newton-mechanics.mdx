...
2. **Law of Inertia (proto-form)**  
   ...
   $$
   \frac{d \boldsymbol v }{dt} = 0 \quad \text{if } \mathbf F  = 0
   $$  
...
$$
\frac{d \mathbf v }{dt} = 0 \quad \Rightarrow \quad \mathbf v  = \text{constant}
$$

Equivalently, in terms of position:

$$
\mathbf r(t)  = \mathbf r_{0}  + \mathbf v_{0} t 
$$
...
Take the Lagrangian $L = T - V = \tfrac{1}{2} m \dot{\vec r}^2 - V(\vec r)$.  
...
$$
m \ddot{\mathbf r} + \nabla V = 0 \quad \Rightarrow \quad m \ddot{\mathbf r} = \mathbf F
$$
...
$$
E = \dot{\mathbf r} \cdot \frac{\partial L}{\partial \dot{\mathbf r}} - L
$$

is conserved. For a particle:

$$
E = \tfrac{1}{2} m v^2 + V(\mathbf r) = \text{constant}
$$
...
$$
m_1 \ddot{\mathbf r}_1 = \mathbf F_{12}, \quad m_2 \ddot{\mathbf r}_2 = \mathbf F_{21}
$$

Adding:

$$
m_1 \ddot{\mathbf r}_1 + m_2 \ddot{\vec r}_2 = \mathbf F_{12} + \mathbf F_{21} = 0
$$

So:

$$
\frac{d}{dt}(m_1 \dot{\mathbf r}_1 + m_2 \dot{\mathbf r}_2) = 0
$$

Thus **total momentum $\vec P$ is conserved**.  
This is the mathematical heart of collisions and recoil.

---

### Space-Translation Symmetry → Momentum Conservation
If the Lagrangian is invariant under $\mathbf r \to \mathbf r + \epsilon$, then:

$$
\mathbf P = \frac{\partial L}{\partial \dot{\mathbf r}} = m \mathbf v
$$
...
But if $\mathbf F \parallel \mathbf r$, then $\mathbf r \times \mathbf F = 0$, so:
...
- $-2 \mathbf \Omega \times \mathbf v'$ = Coriolis force  
...
- **Space translation invariance → momentum conservation**  
  $$\mathbf P = \sum m_i \mathbf v_i = \text{const.}$$  
