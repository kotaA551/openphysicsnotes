...
2. **Law of Inertia (proto-form)**  
   ...
   $$
   \frac{d\vec v}{dt} = 0 \quad \text{if } \vec F = 0
   $$  
...
$$
\frac{d\vec v}{dt} = 0 \quad \Rightarrow \quad \vec v = \text{constant}
$$
...
$$
m \ddot{\vec r} + \nabla V = 0 \quad \Rightarrow \quad m \ddot{\vec r} = \vec F
$$
...
$$
E = \dot{\vec r} \cdot \frac{\partial L}{\partial \dot{\vec r}} - L
$$

is conserved. For a particle:

$$
E = \tfrac{1}{2} m v^2 + V(\vec r) = \text{constant}
$$
...
$$
m_1 \ddot{\vec r}_1 = \vec F_{12}, \quad m_2 \ddot{\vec r}_2 = \vec F_{21}
$$

Adding:

$$
m_1 \ddot{\vec r}_1 + m_2 \ddot{\vec r}_2 = \vec F_{12} + \vec F_{21} = 0
$$

So:

$$
\frac{d}{dt}(m_1 \dot{\vec r}_1 + m_2 \dot{\vec r}_2) = 0
$$
...
If the Lagrangian is invariant under $\vec r \to \vec r + \epsilon$, then:

$$
\vec P = \frac{\partial L}{\partial \dot{\vec r}} = m \vec v
$$
...
But if $\vec F \parallel \vec r$, then $\vec r \times \vec F = 0$, so:
...
- $-2 \vec \Omega \times \vec v'$ = Coriolis force  
- $-\vec \Omega \times (\vec \Omega \times \vec r)$ = Centrifugal force  
...
- **Space translation invariance → momentum conservation**  
  $$\vec P = \sum m_i \vec v_i = \text{const.}$$  

- **Rotation invariance → angular momentum conservation**  
  $$\vec L = \vec r \times m \vec v = \text{const.}$$  
