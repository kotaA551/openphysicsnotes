# Chapter 6: Early Quantum Theory
...
Take a cubic cavity with perfectly conducting walls. Electromagnetic standing waves fit only certain wavevectors $\mathbf k$; each allowed mode is a harmonic oscillator. Counting modes with frequencies between $\nu$ and $\nu+d\nu$ gives a **density of states** per unit volume

$$
g(\nu)\,d\nu = \frac{8\pi \nu^{2}}{c^{3}}\,d\nu
$$

The $8\pi$ comes from spherical shells in $\mathbf k$-space and from two polarization states. This factor is pure geometry/kinematics and will be the same in classical and quantum derivations. The physics enters when we decide how much average energy to assign to **each** mode at equilibrium.
...
Numerically, the handy constant is $e^{2}/(4\pi\varepsilon_{0}) \approx 1.44\ \text{eV·nm}$. For $z=2$ alphas on gold ($Z=79$), $k \approx 79\times 2\times 1.44\ \text{eV·nm} \approx 227.5\ \text{eV·nm}$. With $E=5\ \text{MeV}$,

$$
r_{\min} \approx \frac{227.5\ \text{eV·nm}}{5\times 10^{6}\ \text{eV}} \approx 4.6\times 10^{-5}\ \text{nm} = 4.6\times 10^{-14}\ \text{m}
$$
...
The two equalities say: if something carries energy–momentum $(E,\mathbf p)$, there is a wave with angular frequency $\omega$ and wavevector $\mathbf k$ marching in step.

---

### 6.6.2 Phase and group velocities

For a free particle with dispersion relation $E(\mathbf p)$, define

$$
\omega = \frac{E}{\hbar},\qquad \mathbf k = \frac{\mathbf p}{\hbar}
$$
...
Take the free-particle dispersion $E=p^{2}/(2m)$ and the de Broglie dictionary $E\to i\hbar\,\partial_t$, $\mathbf p\to -i\hbar\nabla$. Demand a linear differential equation whose plane-wave solutions have $\omega=\hbar k^{2}/(2m)$. You land on

$$
i\hbar\,\frac{\partial \psi}{\partial t} = -\,\frac{\hbar^{2}}{2m}\,\nabla^{2}\psi + V\psi
$$

the time-dependent **Schrödinger equation**. In other words, de Broglie’s $E(\mathbf p)$ is the dispersion law that Schrödinger’s PDE realizes in real space.
...
In a uniform magnetic field $\mathbf{B}=B\hat{\mathbf{z}}$, the interaction energy is

$$
H_Z = -\,\boldsymbol{\mu}\cdot\mathbf{B} = g_s\,\mu_B\,m_s\,B
$$
...
Classically flavored but quantum-exact, spins also **precess** about $\mathbf{B}$ at the **Larmor frequency**
...
In atoms, an electron moving in the nuclear field sees, in its rest frame, a magnetic field that couples $\mathbf{L}$ and $\mathbf{S}$:
...
You might ask: why not a spin split with **electrons**? Free electrons in inhomogeneous fields feel huge Lorentz forces $q\,\mathbf{v}\times\mathbf{B}$ and electric-field forces from fringe fields, which swamp the tiny $\nabla(\boldsymbol{\mu}\cdot\mathbf{B})$ splitting. Neutral atoms dodge that, so Stern–Gerlach reached the quantum without EM steering drama.
...
- Starting from $H_Z=-\boldsymbol{\mu}\cdot\mathbf{B}$ and $S_i=\tfrac{\hbar}{2}\sigma_i$, show that spinors precess with $\omega_L=g_s\mu_B B/\hbar$  
- Using vector addition rules, compute $g_J$ for a given term symbol $^{2S+1}L_J$ and predict Zeeman components and polarizations  
- Show how $H_{SO}=\xi(r)\,\mathbf{L}\cdot\mathbf{S}$ splits a given $n,\ell$ multiplet into $J=\ell\pm \tfrac12$ and sketch selection-rule-allowed lines  
...
In wave language, stationary states $\phi_n$ are energy eigenfunctions. Time dependence is a global phase $e^{-iE_n t/\hbar}$, so **expectation values in a stationary state are time-independent**. Driving the system with a weak periodic field $H'=-\hat d\cdot \mathbf E_0\cos\omega t$ and using **time-dependent perturbation theory** yields **transition rates** proportional to matrix elements $\langle m|\hat d|n\rangle$ and a resonance at $\hbar\omega=E_m-E_n$. The dipole operator enforces selection rules like $\Delta \ell=\pm 1$, matching old empirical rules while giving the intensities from first principles.
