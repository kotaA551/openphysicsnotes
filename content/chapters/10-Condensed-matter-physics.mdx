...
For a band $\epsilon(\mathbf k)$ in fields $(\mathbf E,\mathbf B)$ the semiclassical dynamics are

$$
\hbar\,\dot{\mathbf k} = -e\left(\mathbf E + \dot{\mathbf r}\times \mathbf B\right),\qquad
\dot{\mathbf r} = \frac{1}{\hbar}\,\nabla_{\mathbf k}\epsilon(\mathbf k)
$$

With $\mathbf E=0$ you get **cyclotron motion** in $k$-space along constant-energy contours perpendicular to $\mathbf B$; real-space velocity is $\mathbf v=\dot{\mathbf r}$. We postpone Berry curvature and anomalous velocity to §10.11.
...
where the **cyclotron mass** is set by the extremal $k$-space orbit area $A(E)$ normal to $\mathbf B$:
...
Linearizing the Boltzmann equation in a steady field with relaxation time $\tau(\mathbf k)$ gives the current

$$
\mathbf J = -e \int \frac{d^{3}k}{4\pi^{3}}\,\mathbf v(\mathbf k)\,\delta f(\mathbf k),\qquad
\delta f = -\left(-\frac{\partial f_{0}}{\partial \epsilon}\right)\,\boldsymbol{\Lambda}\cdot \mathbf E
$$

In **Chambers’ formula** the vector mean free path is a history average along the orbit

$$
\boldsymbol{\Lambda}(\mathbf k) = \int_{0}^{\infty} dt\; e^{-t/\tau(\mathbf k_{-t})}\,\mathbf v(\mathbf k_{-t})
$$

leading to the conductivity tensor (schematically)

$$
\sigma_{\alpha\beta} = \frac{e^{2}}{4\pi^{3}}\int_{\mathrm{FS}} \frac{dS}{\hbar |\mathbf v|}\, v_{\alpha}\,\bar v_{\beta}
$$
...
Define **mobility** $\boldsymbol{\mu}=\tau\,\mathbf m^{\ast -1}\,e$ and **Hall angle** $\tan\theta_{H}=\sigma_{xy}/\sigma_{xx}\approx \omega_{c}\tau$ (single band). The **mean free path**
...
- **AMRO** (angle-dependent MR oscillations) in layered metals arise when tilting $\mathbf B$ threads Landau orbits across a warped quasi-2D FS; minima occur at **Yamaji angles** set by interlayer warping.

---

### 10.7.8 Quantum oscillations: Onsager + Lifshitz–Kosevich

Landau quantization turns orbit areas into discrete levels. The **Onsager relation** ties the oscillation frequency $F$ (in $1/B$) to the extremal FS cross-section area $A_{F}$ perpendicular to $\mathbf B$:
...
An electron carries spin $\mathbf S$ and possibly orbital $\mathbf L$ contributions. The magnetic moment operator is

$$
\boldsymbol{\mu} = -g_{S}\mu_{B}\,\mathbf S/\hbar - g_{L}\mu_{B}\,\mathbf L/\hbar
$$
...
The **magnetization** $\mathbf M$ is moment per volume. Linear response defines susceptibility $\chi$ via $\mathbf M=\chi \mathbf H$ (isotropic case), with $\mathbf B=\mu_{0}(\mathbf H+\mathbf M)$.

---

### 10.8.2 Local-moment models: Ising/Heisenberg/XXZ

For localized spins $\{\mathbf S_{i}\}$ on a lattice the minimal Hamiltonians are
...
$$
H = -\sum_{\langle ij\rangle} J_{ij}\,\mathbf S_{i}\cdot \mathbf S_{j} - g\mu_{B} B \sum_{i} S_{i}^{z}
$$
...
$$
H \approx E_{0} + \sum_{\mathbf k} \hbar \omega_{\mathbf k}\, b_{\mathbf k}^{\dagger} b_{\mathbf k}
$$

with dispersion near $\mathbf k=0$

$$
\hbar \omega_{\mathbf k} \approx D\,k^{2} + \Delta_{\mathrm{ani}}
$$
...
$$
\hbar \omega_{\mathbf k} \approx c\,|\mathbf k|
$$
...
- **Demagnetizing fields** arise because $\nabla\cdot \mathbf M$ at surfaces acts like “magnetic charge.” Shape defines a demag tensor $\mathsf N$ with field $\mathbf H_{d}=-\mathsf N \mathbf M$.  
...
$$
\frac{d\mathbf M}{dt} = -\gamma\,\mathbf M\times \mathbf H_{\mathrm{eff}} + \frac{\alpha}{M_{s}}\,\mathbf M\times \frac{d\mathbf M}{dt}
$$
...
- **Sign conventions for $J$**: check whether your Hamiltonian uses $-J \mathbf S_{i}\cdot \mathbf S_{j}$ or $+J$; FM/AFM flip with the choice.  
...
Phenomenology first. The **London equations** for the supercurrent $\mathbf J_{s}$ are

$$
\frac{\partial \mathbf J_{s}}{\partial t} = \frac{n_{s} e^{2}}{m}\,\mathbf E
$$

$$
\mathbf J_{s} = -\frac{1}{\mu_{0}\lambda^{2}}\,\mathbf A,\qquad
\lambda^{2} \equiv \frac{m}{\mu_{0} n_{s} e^{2}}
$$

Combine with Maxwell to eliminate $\mathbf J_{s}$ and get **magnetic field screening**
...
GL is a Landau free energy for a complex order parameter $\psi(\mathbf r)$ (Cooper-pair wavefunction) minimally coupled to the vector potential:
...
$$
\mathbf J_{s} = \frac{2e}{m^{\ast}}\,\mathrm{Re}\left\{\psi^{\ast}\left(-i\hbar \nabla - 2e\mathbf A\right)\psi\right\}
$$
...
$$
\oint \left(\nabla \varphi - \frac{2e}{\hbar}\mathbf A\right)\cdot d\boldsymbol\ell = 2\pi n
$$
...
- Starting from the London equations, derive $\nabla^{2}\mathbf B=\mathbf B/\lambda^{2}$ and show exponential decay of $B(x)$ in a half-space  
...
$$
H_{\text{eff}} = J \sum_{\langle i j\rangle} \mathbf S_{i}\cdot \mathbf S_{j},\qquad
J = \frac{4 t^{2}}{U}
$$
...
+ J \sum_{\langle i j\rangle} \left(\mathbf S_{i}\cdot \mathbf S_{j} - \frac{1}{4}\,n_{i} n_{j}\right)
$$
...
Define the one-electron spectral function $A(\mathbf k,\omega)$. In a correlated metal near a Mott transition:
...
**Dynamical Mean-Field Theory (DMFT)** maps a lattice model to a **quantum impurity** embedded in a self-consistent bath. The self-energy is taken **local** (momentum-independent): $\Sigma(\mathbf k,\omega)\to \Sigma(\omega)$—exact in infinite coordination, remarkably good in 3D.
...
For a Bloch band $|u_{n\mathbf k}\rangle$ smooth in $\mathbf k$:

- **Berry connection**

$$
\mathbf A_{n}(\mathbf k) \equiv i \langle u_{n\mathbf k} | \nabla_{\mathbf k} u_{n\mathbf k} \rangle
$$

- **Berry curvature**

$$
\boldsymbol\Omega_{n}(\mathbf k) \equiv \nabla_{\mathbf k} \times \mathbf A_{n}(\mathbf k)
$$

- **Berry phase** along a closed loop $\mathcal C$ in the Brillouin zone (BZ)

$$
\gamma_{n}[\mathcal C] = \oint_{\mathcal C} \mathbf A_{n}\cdot d\mathbf k
$$

Gauge note: $\mathbf A$ depends on phase choice of $|u\rangle$, $\boldsymbol\Omega$ and loop phases modulo $2\pi$ do not. In 1D, the Berry phase of occupied bands controls **polarization** (modern theory): $P$ is a Berry phase over the BZ (see §10.5.10).
...
$$
C_{n} = \frac{1}{2\pi} \int_{\mathrm{BZ}} d^{2}k\; \Omega_{n,z}(\mathbf k)
$$
...
$$
\sigma_{xy}^{\mathrm{int}} = -\frac{e^{2}}{\hbar} \sum_{n} \int \frac{d^{3}k}{(2\pi)^{3}} f_{n\mathbf k}\,\Omega_{n,z}(\mathbf k)
$$
...
For a two-band, chiral-symmetric 1D model $H(k)=\mathbf d(k)\cdot\boldsymbol\sigma$ with $d_{z}=0$, define $\phi(k)=\arg[d_{x}+id_{y}]$. The **winding number**

$$
\nu = \frac{1}{2\pi} \int_{-\pi/a}^{\pi/a} dk\; \partial_{k}\phi(k)
$$

counts how many times $\mathbf d$ wraps the origin. Bulk $\nu=1$ gives **edge zero modes** in the SSH chain (§10.5.3).
...
$$
H(\mathbf k)=v(k_{x}\sigma_{x}+k_{y}\sigma_{y}) + m\,\sigma_{z}
$$
...
- **Weyl nodes** are monopoles of Berry curvature in 3D with integer **chiral charge** $C=\pm 1$. They require breaking either TRS or inversion to separate nodes in $k$-space. Consequences: **Fermi arcs** on surfaces and the **chiral anomaly** (longitudinal negative MR when $\mathbf E\parallel \mathbf B$). The anomaly equation reads

$$
\partial_{t}\rho_{5} + \nabla\cdot \mathbf j_{5} = \frac{e^{2}}{2\pi^{2}\hbar^{2}}\,\mathbf E\cdot \mathbf B
$$
...
$$
H_{R} = \alpha (\boldsymbol\sigma \times \mathbf k)\cdot \hat{\mathbf z}
$$
...
Discretize $k$ along a closed path $\mathcal C$ and build the **Wilson loop**

$$
W[\mathcal C] = \mathcal P \exp\!\left(i\int_{\mathcal C}\! \mathsf A(\mathbf k)\cdot d\mathbf k\right)
$$

where $\mathsf A$ is the non-Abelian Berry connection of the occupied subspace. Eigenphases of $W$ give **Wannier centers**; their flow vs transverse $k$ reveals Chern ($\nu$ winds by $2\pi C$ per BZ) or $\mathbb{Z}_{2}$ (odd vs even partner-switching).
...
For $H=v(k_{x}\sigma_{x}+k_{y}\sigma_{y})+m\sigma_{z}$, compute $\Omega_{z}(\mathbf k)$ and integrate to show $C=-\tfrac{1}{2}\mathrm{sgn}(m)$ per cone; two inequivalent cones with opposite $m$ give $C=\pm 1$.
...
- **Gauge traps**: $\mathbf A$ is gauge-dependent; only $\boldsymbol\Omega$ and loop phases mod $2\pi$ are physical. Use smooth gauges or Wilson loops for numerics.  
...
In TMD monolayers, SOC locks spin to valley. **Circularly polarized** light addresses $K$ vs $K'$ selectively, enabling **valley polarization**. With broken inversion in bilayers (or applied displacement field), you can steer **valley Hall** currents transverse to $E$ without $B$. The **nonlinear Hall effect** emerges from a finite Berry curvature dipole $\mathbf D$:

$$
\mathbf J^{(2\omega)} \propto \tau\, \mathbf D \times \mathbf E(\omega)^{2}
$$
